...
本次分享专为**前端工程师零基础转全栈**打造，完全遵循 NestJS 官方设计规范，摒弃网上碎片化错误教程。

核心目标：搞懂 Nest 分层架构、请求全链路生命周期、各组件职责边界，学完可独立开发 BFF 层、后台 CRUD 接口，掌握企业级开发规范与避坑技巧。

适配人群：前端开发、NestJS 零基础、想要低成本转全栈的工程师

技术栈：NestJS \+ TypeScript（纯前端友好语法，无陌生后端语法）
...
和 Express/Koa 最大区别：**强架构、强分层、依赖注入、工程化约束极强**，天然适合团队协作、中大型项目，也是前端转全栈的最优框架。
...
核心特性：**默认单例模式、全局唯一实例、节省内存、跨组件共享服务能力**
...
- **守卫（Nest内核AOP）**：拥有完整Nest上下文、**完美支持DI依赖注入、可读取接口装饰器元数据**，能实现接口级、角色级的精细化权限管控，是权限校验唯一规范方案
...
企业开发强制标配：**所有接口入参必须通过 Pipe\+DTO 统一校验**，彻底替代手写大量if判断校验，杜绝脏数据、非法参数进入业务逻辑，保证接口稳定性。
...
- **守卫**：专属权限层，唯一负责登录态、角色、接口权限拦截放行

- **管道**：专属参数层，唯一负责入参校验、数据清洗、参数转换
...
```typescript
// main.ts：指定源，不要生产环境裸 enableCors() 放开所有域名
app.enableCors({
  origin: ['http://localhost:5173', 'https://your-frontend.com'],
  methods: ['GET', 'POST', 'PATCH', 'DELETE'],
  allowedHeaders: ['Content-Type', 'token', 'Authorization'],
  credentials: true, // 要带 cookie 时必须写具体 origin，不能用 *
});
```
...
1、【被调用模块】：在 **@Module 装饰器的 exports 数组**中导出需要共享的 Service

2、【调用模块】：在自身 @Module 的 **imports 数组**中导入被调用的业务模块

3、直接在当前模块 Service/Controller 中依赖注入，即可调用跨模块方法，全程无需手动实例化

- 登录接口：校验账号密码，生成 JWT token 返回前端

- 全局守卫：拦截所有请求，校验 token 有效性

- 自定义装饰器：解析 token，快速获取当前用户

### 5\.4 中间件综合实战

统一请求日志、跨域预处理、白名单接口放行、未登录拦截预处理
...
NestJS 对前端开发者极度友好，基于 TS 语法、分层清晰、约束规范。掌握九大核心组件的**执行顺序、职责边界、使用场景**，即可完全胜任 BFF 层、后台接口开发，顺利完成前端到全栈的转型。
